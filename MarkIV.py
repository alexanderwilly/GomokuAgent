...
Coord = Tuple[int, int]
DIRS: List[Tuple[int, int]] = [(1, 0), (0, 1), (1, 1), (1, -1)]  # V, H, ↘, ↙
INF = 10_000_000
...
class Mark4Agent(Agent):
    ...
    async def get_move(self, game_state: GameState) -> Coord:
        ...
        # 2) Must block opponent's immediate win(s)
        ...
        # 4) LLM tie-breaker (tiny prompt, satisfies assignment)
        ...
        # 5) Iterative deepening alpha-beta (time bounded)
        start = time.time()
